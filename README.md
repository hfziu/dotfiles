...
I previously used [Bitwarden](https://bitwarden.com/help/ssh-agent/) as my default SSH agent, setting `SSH_AUTH_SOCK` in `.profile` and `config.fish` to point to Bitwarden's agent socket.

As of May 2026, I switched back to the operating system's default SSH agent. On recent macOS versions, the built-in agent is managed by `launchd`, and `SSH_AUTH_SOCK` is set automatically unless overridden. So I removed all [SSH agent-related configuration](https://github.com/hfziu/dotfiles/commit/e1ad74255a7637c093b65a0b2a00a6d2da75011e) from this repository.

I now use KeePass for SSH key management. Unlike Bitwarden, KeePass is not a standalone SSH agent; it adds or removes keys in the default SSH agent.
